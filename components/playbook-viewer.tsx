...
export function PlaybookViewer() {
  return (
    <div className="mega-panel rounded-[24px] p-6 max-h-[calc(100vh-220px)] overflow-y-auto scrollbar-thin mega-shimmer">
      {/* Header */}
      <div className="sticky top-0 -mx-6 px-6 pb-4 mb-5 border-b border-border bg-card/95 backdrop-blur-md z-10">
      ...
      <ReactMarkdown
        ...
        components={{
          h1: ({ children, ...props }) => (
            <h1
              className="text-xl font-display text-foreground mb-4 mt-6 pb-3 border-b border-border"
              {...props}
            >
              {children}
            </h1>
          ...
          h2: ({ children, ...props }) => (
            <h2
              className="text-lg font-display text-foreground mb-3 mt-6 flex items-center gap-2"
              {...props}
            >
          ...
          blockquote: ({ children, ...props }) => (
            <blockquote
              className="relative border-l-2 border-accent/40 pl-4 pr-4 py-3 my-4 bg-accent/5 rounded-r-xl text-sm text-foreground/80 italic"
              {...props}
            >
              {children}
            </blockquote>
